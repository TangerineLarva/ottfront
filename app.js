...
const bodyParser = require('body-parser');		//data parsing library
const path = require('path');					//url 경로 handling
const axios = require('axios');					//http로 CRUD request library(POST, GET, PUT, DELETE)
const jwt = require('jsonwebtoken');			//JWT(JSON Web Token)처리 library
const cookieParser = require('cookie-parser');	//cookie parsing처리
const fs = require('fs');						//file 읽끼, 쓰기 library
const util = require(__dirname+'/util');		//개발자 libray
//----------

//--- global constants & 환경변수
global.__BASEDIR = __dirname + '/';
global.__ACCESS_TOKEN_NAME = "x-access-token";
global.__AUTH_API_URI = process.env.AUTH_API_URI || "http://hklee.mvp-sample-login.169.56.70.205.nip.io";
